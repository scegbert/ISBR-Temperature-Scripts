function [kappa, wavenumbersCabs] = calcGasKappasByEtaRevB(gasName, pressure, gasT, conc, ...
    etaLow, etaHigh)

% Author: John Tobiasson
% Date: 04/07/2016
% Purpose: This function will caclculate an array of gas emissivities for a
% given set of conditions, from John Person's Cabs data. The resulting array 
% will be passed to another program. The Cabs used will be created from 
...
% This allows the function to interpolate based on pressure, tempearture,
% and concentration (molar). These values are based on what is available
% in Pearson's database.

tLow = 100 .* floor(gasT ./ 100); % determine bounding temperatures
tHigh = 100 .* ceil(gasT ./ 100); %database goes in increments of 100
...
%% convert Cabs to kappa, prepare to return values to calling function
...
kappa = Cabs .* ((calcConc * calcPressure) ./ (Ru .* gasT)) * Na;
